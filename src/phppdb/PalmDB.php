...
// Double conversion
	...
	// Key = record ID
	public $RecordAttrs = array();  // And their attributes are here
	public $RecordSizes = array();  // Used to check record sizes -- don't use!
	public $CurrentRecord = 1;  // Which record we are currently editing
	public $Name = '';  // Name of the PDB file
	public $TypeID = '';  // The 'Type' of the file (4 chars)
	public $CreatorID = '';  // The 'Creator' of the file (4 chars)
	public $Attributes = 0;  // Attributes (bitmask)
	public $Version = 0;  // Version of the file
	public $ModNumber = 0;  // Modification number
	public $CreationTime = 0;  // Stored in unix time (Jan 1, 1970)
	public $ModificationTime = 0;  // Stored in unix time (Jan 1, 1970)
	...
	public $CategoryList = array();  // Category data (not used by default -- See "Category Support" comment below)
	...
	/* Special string loading function
	 * Grabs up to $maxLen characters as part of the string
	 * Will stop at the first NULL character */
	function LoadString($str, $maxLen) {
		$i = 0;
	...
	function CreateCategoryData($IncludeTrailingByte = true) {
		$UsedIds = array();
		$UsedIdCheck = 0;
	...
	function MakeHeader() {
		...
		// Dump each record's header information
		$this->RecordSizes = array();
		...
		if ($this->GetRecordCount() != 0) {
			...
			foreach ($keys as $index) {
				...
				$this->RecordSizes[$index] = $this->GetRecordSize($index);
				$Offset += $this->RecordSizes[$index];
	...
	function WriteToFile($file) {
		...
		foreach ($keys as $index) {
			...
			if (strlen($data) / 2 != $this->RecordSizes[$index]) {
				echo 'Error with class:  Record ' . $index . ' header size (' . $this->RecordSizes[$index] . ') does not match real size (' . (strlen($data) / 2) . ")\n";
			}
